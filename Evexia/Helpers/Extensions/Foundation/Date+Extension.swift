...
public extension Date {
    ...
    init?(fromString string: String, format: DateFormatType, timeZone: TimeZoneType = .local, locale: Locale = Foundation.Locale.current) {
        ...
        case .dotNet:
            ...
            #endif
            let interval = Double(dateString)! / 1_000.0
    ...
    func toString(format: DateFormatType, timeZone: TimeZoneType = .local, locale: Locale = Locale.current) -> String {
        switch format {
        case .dotNet:
            let offset = Foundation.NSTimeZone.default.secondsFromGMT() / 3_600
    ...
    func toStringWithRelativeTime(strings: [RelativeTimeStringType: String]? = nil) -> String {
        ...
        if sec < 60 {
            if sec < 10 {
                if isPast {
                    return strings?[.nowPast] ?? NSLocalizedString("date.helper.just.now", comment: "Date format")
                } else {
                    return strings?[.nowFuture] ?? NSLocalizedString("date.helper.few.seconds", comment: "Date format")
                }
            } else {
                let string: String
                if isPast {
                    string = strings?[.secondsPast] ?? NSLocalizedString("date.helper.count.seconds.ago", comment: "Date format")
                } else {
                    string = strings?[.secondsFuture] ?? NSLocalizedString("date.helper.in.count.seconds", comment: "Date format")
                }
        ...
        if min < 60 {
            if min == 1 {
                if isPast {
                    return strings?[.oneMinutePast] ?? NSLocalizedString("date.helper.one.minute.ago", comment: "Date format")
                } else {
                    return strings?[.oneMinuteFuture] ?? NSLocalizedString("date.helper.in.one.minute", comment: "Date format")
                }
            } else {
                let string: String
                if isPast {
                    string = strings?[.minutesPast] ?? NSLocalizedString("date.helper.count.minutes.ago", comment: "Date format")
                } else {
                    string = strings?[.minutesFuture] ?? NSLocalizedString("date.helper.in.count.minutes", comment: "Date format")
                }
        ...
        if hr < 24 {
            if hr == 1 {
                if isPast {
                    return strings?[.oneHourPast] ?? NSLocalizedString("date.helper.last.hour", comment: "Date format")
                } else {
                    return strings?[.oneHourFuture] ?? NSLocalizedString("date.helper.next.hour", comment: "Date format")
                }
            } else {
                let string: String
                if isPast {
                    string = strings?[.hoursPast] ?? NSLocalizedString("date.helper.count.hours.ago", comment: "Date format")
                } else {
                    string = strings?[.hoursFuture] ?? NSLocalizedString("date.helper.in.count.hours", comment: "Date format")
                }
        ...
        if d < 7 {
            if d == 1 {
                if isPast {
                    return strings?[.oneDayPast] ?? NSLocalizedString("date.helper.yesterday", comment: "Date format")
                } else {
                    return strings?[.oneDayFuture] ?? NSLocalizedString("date.helper.tomorrow", comment: "Date format")
                }
            } else {
                let string: String
                if isPast {
                    string = strings?[.daysPast] ?? NSLocalizedString("date.helper.count.days.ago", comment: "Date format")
                } else {
                    string = strings?[.daysFuture] ?? NSLocalizedString("date.helper.in.count.days", comment: "Date format")
                }
        ...
        if d < 28 {
            if isPast {
                if compare(.isLastWeek) {
                    return strings?[.oneWeekPast] ?? NSLocalizedString("date.helper.last.week", comment: "Date format")
                } else {
                    let string = strings?[.weeksPast] ?? NSLocalizedString("date.helper.count.weeks.ago", comment: "Date format")
                    return String(format: string, Double(abs(since(Date(), in: .week))))
                }
            } else {
                if compare(.isNextWeek) {
                    return strings?[.oneWeekFuture] ?? NSLocalizedString("date.helper.next.week", comment: "Date format")
                } else {
                    let string = strings?[.weeksFuture] ?? NSLocalizedString("date.helper.in.count.weeks", comment: "Date format")
                    return String(format: string, Double(abs(since(Date(), in: .week))))
                }
            }
        }
        if compare(.isThisYear) {
            if isPast {
                if compare(.isLastMonth) {
                    return strings?[.oneMonthPast] ?? NSLocalizedString("date.helper.last.month", comment: "Date format")
                } else {
                    let string = strings?[.monthsPast] ?? NSLocalizedString("date.helper.count.months.ago", comment: "Date format")
                    return String(format: string, Double(abs(since(Date(), in: .month))))
                }
            } else {
                if compare(.isNextMonth) {
                    return strings?[.oneMonthFuture] ?? NSLocalizedString("date.helper.next.month", comment: "Date format")
                } else {
                    let string = strings?[.monthsFuture] ?? NSLocalizedString("date.helper.in.count.months", comment: "Date format")
                    return String(format: string, Double(abs(since(Date(), in: .month))))
                }
            }
        }
        if isPast {
            if compare(.isLastYear) {
                return strings?[.oneYearPast] ?? NSLocalizedString("date.helper.last.year", comment: "Date format")
            } else {
                let string = strings?[.yearsPast] ?? NSLocalizedString("date.helper.count.years.ago", comment: "Date format")
                return String(format: string, Double(abs(since(Date(), in: .year))))
            }
        } else {
            if compare(.isNextYear) {
                return strings?[.oneYearFuture] ?? NSLocalizedString("date.helper.next.year", comment: "Date format")
            } else {
                let string = strings?[.yearsFuture] ?? NSLocalizedString("date.helper.in.count.years", comment: "Date format")
                return String(format: string, Double(abs(since(Date(), in: .year))))
            }
        }
    }
...
public enum DateFormatType {
    ...
    /// The ISO8601 formatted date, time and sec "yyyy-MM-dd HH:mm:ss" i.e. 2018-07-28 00:00:00
    case dateTimeSec
    ...
    /// The ISO8601 formatted date, time and sec "yyyy-MM-dd HH:mm:ss" i.e. 2018-07-28 00:00:00
    case dateMonthAchieved
    
    /// The ISO8601 formatted date, time and sec "yyyy-MM-dd HH:mm:ss" i.e. 2018-07-28 00:00:00
    case dateTransaction
    
    /// The ISO8601 formatted date, time and sec "HH:mm" i.e. 00:00
    case dateTime
    ...
    var stringFormat: String {
        ...
        case .dateMonth: return "d MMM"
        case .dateMonthTime: return "d MMM, HH:mm"
        case .dateMonthAchieved: return "yyyy/MM/dd"
        case .dateTransaction: return "dd/MM/yyyy HH:mm"
        case .dateTime: return "HH:mm"
        }
    }
}
...
public extension Date {
    func plus(days d: UInt) -> Date {
        return self.addComponentsToDate(seconds: 0, days: Int(d), weeks: 0, months: 0, years: 0)
    }
    ...
    func plus(weeks w: UInt) -> Date {
        return self.addComponentsToDate(seconds: 0, days: 0, weeks: Int(w), months: 0, years: 0)
    }
    ...
    func plus(months m: UInt) -> Date {
        return self.addComponentsToDate(seconds: 0, days: 0, weeks: 0, months: Int(m), years: 0)
    }
    ...
    func plus(years y: UInt) -> Date {
        return self.addComponentsToDate(seconds: 0, days: 0, weeks: 0, months: 0, years: Int(y))
    }
    ...
    func plus(seconds s: UInt) -> Date {
        return self.addComponentsToDate(seconds: Int(s), days: 0, weeks: 0, months: 0, years: 0)
    }
    ...
    fileprivate func addComponentsToDate(seconds sec: Int, days d: Int, weeks wks: Int, months mts: Int, years yrs: Int) -> Date {
