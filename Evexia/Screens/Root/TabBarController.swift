...
extension TabBarController: CardTabBarDelegate {
    func cardTabBar(_ sender: BaseCardTabBar, didSelectItemAt index: Int) {
        ...
        guard let navControllers = self.viewControllers?.compactMap({ $0 as? UINavigationController }), let indexVC = navControllers.firstIndex(where: { $0.viewControllers.first is DiaryVC }), index != indexVC, let diaryVC = navControllers[indexVC].visibleViewController as? DiaryVC else { return }
        //TODO: - find best solution (its hot fix, because i still dont understand how it works)
        if index == 0 {
            navControllers[0].popToRootViewController(animated: false)
        }
        diaryVC.dismissAlert()
    }
}
