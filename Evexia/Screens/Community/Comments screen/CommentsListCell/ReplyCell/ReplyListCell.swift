...
// MARK: - ReplyListCell
class ReplyListCell: UITableViewCell, CellIdentifiable {
    ...
    // MARK: - Methods
    @discardableResult
    func config(with model: ReplyModel) -> ReplyListCell {
        ...
        let customType = ActiveType.custom(pattern: "\(model.replyTo.username)")
        
        commentTextLabel.attributedText = model.content.style(tags: [self.userAttributes]).styleAll(self.plainAttributes)
        //commentTextLabel.enabledTypes += [customType]
        //commentTextLabel.customColor[customType] = .orange
        //commentTextLabel.text = model.content
...
private extension ReplyListCell {
    func setupUI() {
